...
public class ModelSubwoofer extends ModelTransformerBase {
	...
	public void doTransformationAnimations(final EntityPlayer player, final float progress, final float limbSwing, final float limbSwingAmount, final float ticks, final float rotationYaw, final float rotationPitch, final boolean wearingHead, final boolean wearingChest, final boolean wearingLegs, final boolean wearingFeet) {
		...
		rotateTo(upperArmL, vehicle.vehicleUpperArmL, progress);
		rotateTo(dish1, vehicle.vehicleDish1, progress);
		rotateTo(dish2, vehicle.vehicleDish2, progress);
		rotateTo(antenna1, vehicle.vehicleAntenna1, progress);
		rotateTo(antenna2, vehicle.vehicleAntenna2, progress);
		rotateTo(lowerlegR1, vehicle.vehicleLowerLegR1, progress);
		rotateTo(lowerlegL1, vehicle.vehicleLowerLegL1, progress);
		rotateTo(upperLegR, vehicle.vehicleUpperLegR, progress);
		rotateTo(upperLegL, vehicle.vehicleUpperLegL, progress);
		rotateTo(lowerArmR, vehicle.vehicleLowerArmRL1, progress);
		rotateTo(lowerArmL, vehicle.vehicleLowerArmL1, progress);
		rotateTo(chestmain1, vehicle.vehicleChestMain1, progress);
		rotateTo(chestmain2, vehicle.vehicleChestMain2, progress);
		rotateTo(chestmain3, vehicle.vehicleChestMain3, progress);
		rotateTo(chestmain5, vehicle.vehicleChestMain5, progress);
		rotateTo(chestmain6, vehicle.vehicleChestMain6, progress);
		rotateTo(frontchestR, vehicle.vehicleFrontChest1, progress);
		rotateTo(frontchestL, vehicle.vehicleFrontChest2, progress);
		rotateTo(bass2, vehicle.vehicleBass2, progress);
		rotateTo(bass3, vehicle.vehicleBass3, progress);
		rotateTo(crotch1, vehicle.vehicleCrotch1, progress);
		rotateTo(lowerlegL3, vehicle.vehicleLowerLegL3, progress);
		rotateTo(lowerlegR3, vehicle.vehicleLowerLegR3, progress);
		rotateTo(stomach, vehicle.vehicleStomach, progress);
		rotateTo(shoulderbaseR, vehicle.vehicleShoulderBase1, progress);
		rotateTo(shoulderbaseL, vehicle.vehicleShoulderBase2, progress);
		shoulderplateR1.rotationPointX += progress * 0.2F;
		shoulderplateL1.rotationPointX -= progress * 0.2F;
		shoulderplateR1.rotationPointZ -= progress * 0.2F;
		shoulderplateL1.rotationPointZ -= progress * 0.2F;
		head.rotationPointY += progress * 0.2F;
	}
